...
export async function ScipdfParser(pdf_src: string | Buffer, options: ScipdfOptions) {
	if (!pdf_src) {
...
function parseTEIXml(xml: string, options: ScipdfOptions) {
	const article: ScipdfArticle = {
		title: '',
		authors: [],
		orgs: [],
		pub_date: '',
	...
	if (ind1 < 0) throw new Error('failed to parse')
	let doc = new DOMParser().parseFromString(xml.substring(ind1, xml.indexOf('</teiHeader>') + 12))
	const select = useNamespaces({})
	const selectSingle = (exp: string, attr?: string) => {
		const el = select(exp, doc, true) as Element
		if (el) {
			return ((attr ? el.getAttribute(attr) : el.textContent) || '').trim()
		}
		return ''
	}
	const selectNodes = (exp: string) => select(exp, doc, false) as Element[]
...
function child2Array(node: Element): Element[] {
	return node.childNodes ? Object.keys(node.childNodes).map(n => node.childNodes[n as any] as Element) : []
}

function cell2text(node: Element, list: string[] = []) {
...
function getNodeText(nodes: Element[]): string {
	let text = ''
